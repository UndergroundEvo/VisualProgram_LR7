...
namespace GraphicEditor.Models
{
    public class Mapper
    {
        ...
        internal void SetProp(PropsN num, object obj)
        {
            ...
            {
                case PName: 
                    shapeName = (string)obj; 
                    break;

                case PColor: 
                    shapeColor = (string)obj; 
                    break;

                case PFillColor: 
                    shapeFillColor = (string)obj; 
                    break;

                case PThickness: 
                    shapeThickness = (int)obj; 
                    break;
            };
        }
        ...
        public Shape[]? Import(bool is_xml, object? content = null)
        {
            ...
            foreach (object? item in @list)
            {
                ...
                { 
                    Log.Write("Нет поля type, либо оно - не строка"); 
                    continue; 
                }

                if (!@dict.ContainsKey("name") || @dict["name"] is not string @shapeName) 
                { 
                    Log.Write("Нет поля name, либо оно - не строка"); 
                    continue; 
                }
                ...
                if (newy == null) 
                { 
                    Log.Write("Не получилось собрть фигуру " + Utils.Obj2json(@dict)); 
                    continue; 
                }
                ...
                if (@dict.TryGetValue("transform", out object? tform))
                    if (tform is not Dictionary<string, object?> @dict2) 
                        ...
                        Transformator.Import(newy, @dict2);
        ...
        public void Select(ShapeListBoxItem? shapeItem)
        {
            ...
            if (yeah)
            {
                if (shape.Name != null && shape.Name.StartsWith("sn_")) 
                    SetProp(PName, shape.Name[3..]);
                ...
                Log.Write("Не удалось распаковать фигуру :/");
        }
        ...
        public void PressShape(Shape shape, Point pos)
        {
            ...
            if (old_pos == null) 
            { 
                Log.Write("Не удалось считать позицию фигуры :/"); 
                return; 
            }
        ...
        public void MoveShape(Shape shape, Point pos)
        {
            ...
            if (!yeah) 
            { 
                Log.Write("Не удалось переместить фигуру :/");
                return; 
            }
            ...
            if (shape.Name == "sn_marker")
            {
                ...
                if (yeah)
                {
                    ...
                    Log.Write("Не удалось распотрошить фигуру :/");
