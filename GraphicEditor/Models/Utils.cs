    ...
    public static class Utils
    {
        ...
        private static object? JsonHandler(object? obj)
        {
            ...
            if (obj is Dictionary<string, object?> @dict) 
                return @dict.Select(pair => new KeyValuePair<string, object?>(pair.Key, JsonHandler(pair.Value)));
        ...
        private static string List2XML(List<object?> list, string level)
        {
            ...
            foreach (var entry in list)
                if (IsComposite(entry)) 
                    ...
                    attrs.Append(" " + ToXMLHandler(entry, "{err}") + "=''");
        ...
        {
            if (str.Length > 1 && str[0] == '$' && str[1] <= '9' && str[1] >= '0') 
                return str[1..];
