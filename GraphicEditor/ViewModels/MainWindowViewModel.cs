...
namespace GraphicEditor.ViewModels
{
    ...
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public IBrush AddColor 
        { 
            get => add_color; 
            set => this.RaiseAndSetIfChanged(ref add_color, value); 
        }
        ...
        {
            if (inst != null && inst is MainWindowViewModel @mwvm) 
                @mwvm.Update();
        }
        ...
        public MainWindowViewModel(MainWindow mw)
        {
            ...
            canv.PointerPressed += (object? sender, PointerPressedEventArgs e) =>
            {
                if (e.Source != null && e.Source is Shape @shape) 
                    map.PressShape(@shape, e.GetCurrentPoint(canv).Position);
            };

            canv.PointerMoved += (object? sender, PointerEventArgs e) =>
            {
                if (e.Source != null && e.Source is Shape @shape) 
                    map.MoveShape(@shape, e.GetCurrentPoint(canv).Position);
            };
        ...
        public bool DropboxVisible 
        { 
            get => dropbox_visible; 
            set => this.RaiseAndSetIfChanged(ref dropbox_visible, value); 
        }
        ...
        public int SelectedShaper
        {
            ...
            set 
            { 
                this.RaiseAndSetIfChanged(ref shaper_n, value); 
                map.ChangeFigure(value); 
                Content = contentArray[value]; 
            }
        }
        ...
        { 
            get => service_visible; 
            set => this.RaiseAndSetIfChanged(ref service_visible, value);
        }
